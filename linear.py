...
def open_region_anvil(file_path):
    ...
    file_coords = os.path.basename(file_path).split('.')[1]
    match = re.match(r'(\-?\d+)(.*)', file_coords)
    if match:
        region_x, region_z = int(match.group(1)), int(match.group(1))
    ...
    for i in range(REGION_DIMENSION * REGION_DIMENSION):
        if chunk_starts[i] > 0 and chunk_sizes[i] > 0:
            ...
            chunks.append(Chunk(zlib.decompress(whole_raw_chunk[5:5 + chunk_size],15 + 32), REGION_DIMENSION * region_x + i % 32, REGION_DIMENSION * region_z + i // 32))
        else:
            chunks.append(None)

    return Region(chunks, region_x, region_z, mtime, timestamps)

def write_region_anvil(destination_filename, region: Region, compression_level=6):
    SECTOR = 4096
    ...
    for i in range(REGION_DIMENSION * REGION_DIMENSION):
        ...
        if region.chunks[i] is not None:
            compressed = gzip.compress(region.chunks[i].raw_chunk, compression_level)
            ...
            if sector_count > 255:
                ...
                with open(chunk_file_path + ".wip", "wb") as chunk_file:
...
def write_region_anvil_to_bytes(region: Region, compression_level=6):
    SECTOR = 4096
